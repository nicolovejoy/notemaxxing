...
export async function GET(_request: NextRequest) {
  try {
    ...
    // 4. Get total counts
    const [foldersResult, notebooksResult, notesResult, permissionsResult, invitationsResult] =
      await Promise.all([
    ...
    const { data: notebookCreators } = await serviceClient.from('notebooks').select('owner_id')

    notebookCreators?.forEach((item) => {
      creatorCounts.set(item.owner_id, (creatorCounts.get(item.owner_id) || 0) + 1)
    })

    const { data: noteCreators } = await serviceClient.from('notes').select('owner_id')
    ...
    return NextResponse.json({
      ...
      averages: {
        folders_per_user:
          totalUsers > 0 ? ((foldersResult.count || 0) / totalUsers).toFixed(1) : '0',
        notebooks_per_user:
          totalUsers > 0 ? ((notebooksResult.count || 0) / totalUsers).toFixed(1) : '0',
