...
export async function GET(request: Request) {
  ...
  // Get owned folders
  const foldersSnap = await db.collection('folders').where('owner_id', '==', uid).orderBy('name').get()
  ...
  const foldersWithNotebooks = folders.map(folder => ({
    ...
    note_count: (notebooksByFolder[folder.id] || []).reduce((s, nb) => s + (nb.note_count as number), 0),
