-- Admin functions for Notemaxxing (v2 - with new seed data design)
-- These functions help with admin operations in the admin console

-- Function to get user statistics (this one is needed by admin console)
...
BEGIN
  RETURN QUERY
  SELECT 
    (SELECT COUNT(*) FROM folders WHERE user_id = target_user_id)::BIGINT,
    (SELECT COUNT(*) FROM notebooks WHERE user_id = target_user_id)::BIGINT,
    (SELECT COUNT(*) FROM notes WHERE user_id = target_user_id)::BIGINT,
    (SELECT COUNT(*) FROM quizzes WHERE user_id = target_user_id)::BIGINT;
END;
...
-- Include the create_starter_content_for_specific_user function from seed-new-users-v2.sql
-- (This is the function that the admin console "Seed My Data" button uses)
